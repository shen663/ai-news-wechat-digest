...
上述 Windows 任务以交互登录会话运行：电脑需要在 10 点保持唤醒、联网且已登录（锁屏可以）。若任务因待机而延迟启动，或运行失败，任务计划程序会在失败后每隔 2 分钟最多重试 3 次；若届时仍未联网，当天可能无法自动推送。若要不依赖个人电脑，应把同一项目放在持续运行的主机上，由该主机的定时任务在北京时间 10:00 执行 `python ai_digest.py run`。
